...
void I2C_Init_Master(unsigned long speed_master){
    ...
    TRISB |= 0x03;
    SSPCON1 = 0x28;
    SSPADD = (unsigned char)((48000000UL/(4*speed_master))-1);
    SSPSTAT = 0x80;
...
void WritePCF(uint8_t dato){
    I2C_Start();
    I2C_Write_Data((7<<1)|0x40);
    I2C_Write_Data(dato|0b00001000);
    I2C_Stop();
}
...
void dato_LCD(uint8_t ctrl,uint8_t cmd){
    uint8_t dato;
    if((cmd&1)==1) dato=dato|0b00010000; else dato=dato&~0b00010000;
    if((cmd&2)==2) dato=dato|0b00100000; else dato=dato&~0b00100000;
    if((cmd&4)==4) dato=dato|0b01000000; else dato=dato&~0b01000000;
    if((cmd&8)==8) dato=dato|0b10000000; else dato=dato&~0b10000000;
    _delay((unsigned long)((60)*(48000000UL/4000000.0)));

    if(ctrl==1)
        dato = dato|0b00000001;
    else
        dato = dato&~0b00000001;

    WritePCF(dato|0b00000100);
    _delay((unsigned long)((2)*(48000000UL/4000000.0)));
    WritePCF(dato &~0b00000100);
}

void trama_LCD(uint8_t ctrl, uint8_t cmd){
    dato_LCD(ctrl,cmd>>4);
    dato_LCD(ctrl,cmd&0x0F);
}
